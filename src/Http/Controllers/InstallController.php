...
namespace medvex\WebInstaller\Http\Controllers;
...
use medvex\WebInstaller\Events\EnvironmentSaved;
use medvex\WebInstaller\Http\Helpers\DatabaseManager;
...
class InstallController extends Controller
{
    ...
    public function preInstall()
    {
        ...
        return view('web.installer.pre-install', compact('route_value', 'resource_value', 'public_value', 'storage_value', 'env_value'));
    }
    ...
    public function configuration()
    {
        ...
        if(session()->has('validated')) {
            return view('web.installer.config');
        }
        return redirect(route('WebInstaller::pre-install'));
    }
    ...
    public function serverValidation(Request $request)
    {
        if($this->phpversion() > 7.0 && $this->mysqli() == 1 && $this->curl_version() == 1 && $this->allow_url_fopen() == 1 && $this->openssl() == 1 && $this->pdo() == 1 && $this->bcmath() == 1 && $this->ctype() == 1 && $this->fileinfo() == 1 && $this->mbstring() == 1 && $this->tokenizer() == 1 && $this->xml() == 1 && $this->json() == 1){
            ...
            return redirect(route('WebInstaller::config'));
        }
    ...
    public function final(Request $request)
    {
        ...
        return Redirect::route('WebInstaller::database')
    ...
    public function database()
    {
        ...
        if($response['status'] = 'success') {
            ...
            if (! file_exists($installedLogFile)) {
                $message = trans('WebInstaller successfully INSTALLED on ').$dateStamp."\n";
